...
class GenerativeAgentMemory(BaseMemory):
    ...
    memory_retriever: TimeWeightedVectorStoreRetriever  # 检索相关记忆
    """The retriever to fetch related memories."""
    verbose: bool = False

    reflection_threshold: Optional[float] = None  # 当aggregate_importance超过reflection_threshold时，停止反思。
    ...
    aggregate_importance: float = 0.0  # 跟踪最近记忆的重要性总和 : :meta private:
    ...
    def fetch_memories(
        ...
        if now is not None:
            with mock_now(now):
                return self.memory_retriever.get_relevant_dgetocuments(observation)
        else:
